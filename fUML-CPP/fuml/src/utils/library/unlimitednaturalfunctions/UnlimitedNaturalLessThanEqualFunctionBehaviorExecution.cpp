...
{
	this->types->push_back(fuml::library::FoundationalModelLibraryModel::Instance()->FoundationalModelLibrary_PrimitiveBehaviors_UnlimitedNaturalFunctions_lessOrEqual);
}
	...
	const std::shared_ptr<std::vector<int>>& arguments)
{
	...
	fuml::Debug::println("[doBody] UnlimitedNatural Less Than or Equal result = " + std::to_string(b));
	return b;
}
