...
{
	this->types->push_back(fuml::library::LibraryModel::Instance()->UnlimitedNaturalFunctions_greaterOrEqual);
}
	...
	const std::shared_ptr<std::vector<int>>& arguments)
{
	...
	utils::Debug::println("[doBody] UnlimitedNatural Greater Than or Equal result = " + std::to_string(b));
	return b;
}
