...
void ActionActivation::setThisActionActivationPtr(
		std::weak_ptr<ActionActivation> thisActionActivationPtr)
{
	this->thisActionActivationPtr = thisActionActivationPtr;
	ActivityNodeActivation::setThisActivityNodeActivation(
			thisActionActivationPtr);
}

void ActionActivation::initialize(const ActivityNodePtr &node,
...
TokenListPtr ActionActivation::completeAction()
{
	...
	utils::Debug::println(
			"[fire] Checking if " + this->node->name + " should fire again...");
...
bool ActionActivation::isReady()
{
	...
	const InputPinListPtr &inputPins = std::dynamic_pointer_cast<Action>(
			this->node)->input;
...
bool ActionActivation::isControlReady()
{
	...
	bool ready =
			ActivityNodeActivation::isReady()
					&& (std::dynamic_pointer_cast<Action>(this->node)->isLocallyReentrant
							|| !this->isFiring());
...
void ActionActivation::createNodeActivations()
{
	...
	{
		this->addPinActivation(
				std::dynamic_pointer_cast<PinActivation>(
						this->group->getNodeActivation(node)));
	}
...
void ActionActivation::putTokens(const OutputPinPtr &pin,
...
ValueListPtr ActionActivation::getTokens(const InputPinPtr &pin)
{
	...
	utils::Debug::println(
			"[getTokens] node = " + this->node->name + ", pin = " + pin->name);
...
ValueListPtr ActionActivation::takeTokens(const InputPinPtr &pin)
{
	...
	utils::Debug::println(
			"[takeTokens] node = " + this->node->name + ", pin = " + pin->name);
...
bool ActionActivation::isSourceFor(const ActivityEdgeInstancePtr &edgeInstance)
{
	...
	if (this->outgoingEdges->size() > 0)
	{
		isSource = this->outgoingEdges->at(0)->target->isSourceFor(
				edgeInstance);
	}
...
bool ActionActivation::valueParticipatesInLink(const ValuePtr &value,
		const LinkPtr &link)
{
...
AssociationPtr ActionActivation::getAssociation(
...
ValueListPtr ActionActivation::getValues(const ValuePtr &sourceValue,
		const StructuralFeaturePtr &feature)
{
	...
	{
		LinkListPtr links = this->getMatchingLinks(association, feature,
				sourceValue);
	...
	{
		values =
				std::dynamic_pointer_cast<StructuredValue>(sourceValue)->getFeatureValue(
						feature)->values;
	}
...
LinkListPtr ActionActivation::getMatchingLinks(
		const AssociationPtr &association, const StructuralFeaturePtr &end,
		const ValuePtr &oppositeValue)
{
	...
	return this->getMatchingLinksForEndValue(association, end, oppositeValue,
			nullptr);
} // getMatchingLinks

LinkListPtr ActionActivation::getMatchingLinksForEndValue(
		const AssociationPtr &association, const StructuralFeaturePtr &end,
		const ValuePtr &oppositeValue, const ValuePtr &endValue)
{
	// Get the links of the given binary association whose end opposite
	...
	{
		if (link->getFeatureValue(oppositeEnd)->values->at(0)->equals(
				oppositeValue))
		{
			bool matches = true;
			if (endValue != nullptr)
			{
				matches = link->getFeatureValue(end)->values->at(0)->equals(
						endValue);
			}
			if (matches)
			{
				...
				else
				{
					...
					for (const LinkPtr &link : *links)
					{
						j = j + 1;
						continueSearching =
								(link->getFeatureValue(end)->position < n);
					}
...
PropertyPtr ActionActivation::getOppositeEnd(const AssociationPtr &association,
		const StructuralFeaturePtr &end)
{
...
BooleanValuePtr ActionActivation::makeBooleanValue(bool value)
{
	...
	return std::dynamic_pointer_cast<BooleanValue>(
			this->getExecutionLocus()->executor->evaluate(booleanLiteral));
} // makeBooleanValue

void ActionActivation::handle(const ValuePtr &exception,
		const ExceptionHandlerPtr &handler)
{
	...
	this->transferOutputs(
			std::dynamic_pointer_cast<Action>(handler->handlerBody));
} // handle
...
void ActionActivation::transferOutputs(const ActionPtr &handlerBody)
{
	...
	ActionActivationPtr handlerBodyActivation = std::dynamic_pointer_cast<
			ActionActivation>(this->group->getNodeActivation(handlerBody));
	const OutputPinListPtr &sourceOutputs = handlerBody->output;
	OutputPinListPtr targetOutputs = std::dynamic_pointer_cast<Action>(
			this->node)->output;
	...
	for (unsigned int i = 0; i < sourceOutputsSize; i++)
	{
		...
		PinActivationPtr sourcePinActivation =
				handlerBodyActivation->getPinActivation(sourcePin);
