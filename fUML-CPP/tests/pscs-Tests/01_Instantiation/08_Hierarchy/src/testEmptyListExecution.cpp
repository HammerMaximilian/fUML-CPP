...
testEmptyListExecution::testEmptyListExecution()
{
	this->types->push_back(HierarchyModel::Instance()->Hierarchy_assert_A_testEmptyList);
...
void testEmptyListExecution::doBody(const ParameterValueListPtr& inputParameters,
	const ParameterValueListPtr& outputParameters)
{
	...
	{
		std::cout<<ASCII_RED << "Assertion failed" << ASCII_BLK << " for "<< inputParameters->at(0)->parameter->name << ". Was: " << listSize << " ; Should: 0"<<std::endl;
	}
}

ValuePtr testEmptyListExecution::new_()
{
	std::shared_ptr<testEmptyListExecution> newValue(new testEmptyListExecution());
