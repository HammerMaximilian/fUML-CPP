...
{
	this->types->push_back(SingleDelegation_P_P_SignalModel::Instance()->SingleDelegation_P_P_Signal_Tester_testActivity_testP);
	...
	const ParameterValueListPtr& outputParameters)
{
	...
	std::cout<<"Asserting a.b.p == 4"<<std::endl;

	// Test parameter p
	int p = AS(IntegerValue, inputParameters->at(0)->values->at(0))->value;
	fuml::Debug::println("[doBody] argument p = " + std::to_string(p));
	
	std::cout << "a.b.p == " << std::to_string(p) <<std::endl;
	std::cout<<"Signal delegated to classifier behavior: ";
